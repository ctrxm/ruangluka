  ...
  return (
    <Card className="p-3.5 border border-border">
      <div className="flex items-center gap-1.5 mb-2">
        <Badge variant="secondary" className="text-[10px] font-medium">Iklan</Badge>
      </div>
      {ad.type === "image" && ad.imageUrl && (
        <a href={ad.linkUrl || "#"} target="_blank" rel="noopener noreferrer" data-testid={`link-ad-${ad.id}`}>
          <img src={ad.imageUrl} alt={ad.title} className="w-full rounded-lg mb-2 object-cover max-h-40" />
        </a>
      )}
